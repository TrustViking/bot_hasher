...
class Start(ConfigInitializer):
    ...
    def system_status(self):
        ...
        def _system_status():
            file_start = basename(sys.argv[0]) #  [start_hasher.py]
    ...
    def log_memory(self):
        ...
        def _log_memory():
            ...
            self.logger.log_info(f'*Data RAM {basename(sys.argv[0])}: [{virtual_memory()[2]}%]')
    ...
    def get_names_files(self, patterns_inname: List[str]) -> List[str]:
        @safe_execute(logger=self.logger, name_method=f'[{__name__}|{self.cls_name}]')
        def _get_names_files():
            directory_path = dirname(abspath(__file__))
            nfiles = sorted(listdir(directory_path))
            name_files = [nfile for nfile in nfiles if all(pattern in nfile for pattern in patterns_inname)]
            print(f'\n[{__name__}|{self.cls_name}] name_files: {name_files}')
            return name_files
        return _get_names_files()
    ...
    def cmd_string(self, name_scripts: List[str]) -> Optional[List[str]]:
        ...
        def _cmd_string():
            ...
            cmd_strings = [f'{name_lang} {exist_path}' for exist_path in exist_paths]
    ...
    def bot_scripts(self, patterns_inname: List[str])-> Optional[List[str]]:
        @safe_execute(logger=self.logger, name_method=f'[{__name__}|{self.cls_name}]')
        def _bot_scripts():
            name_bots = self.get_names_files(patterns_inname)
            if not name_bots:
                print(f'\n[{__name__}|{self.cls_name}] ERROR name_bots: {name_bots}')
                return None
            return self.cmd_string(name_bots)
        return _bot_scripts()
...
async def main():
    ...
    await gather(*tasks)
    #
    # Запускаем ботов асинхронно
    scripts = start.bot_scripts(['bot', '.py'])
    tasks = [start.subprocess_script(script) for script in scripts]
    await gather(*tasks)
